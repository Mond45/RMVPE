...
class Inference:
    ...
    def inference(self, audio):
        with torch.no_grad():
            ...
            hidden_vec_segments, out_segments = self.forward_in_mini_batch(
                self.model, segments
            )
            out_segments = self.de_frame(out_segments, type_seg="pitch")[
                : (len(audio) // self.hop_length + 1)
            ]
            hidden_vec_segments = self.de_frame(hidden_vec_segments, type_seg="pitch")[
                : (len(audio) // self.hop_length + 1)
            ]
            ...
            if batch_start + self.batch_size >= segments_num:
                ...
                segment_in = torch.cat(
                    [
                        ...
                        torch.zeros_like(segments)[: self.batch_size - batch_tmp].to(
                            self.device
                        ),
                    ],
                    dim=0,
